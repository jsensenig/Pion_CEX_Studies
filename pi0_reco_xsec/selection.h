...
public:
  ...
  Selection() {
    set_selection_parameters( "../selection_params.json" );
  }
  virtual ~Selection() { ; };
  ...
  inline bool daughter_michel_score( double score ) const {
    return score > lower_michel_score;
  }
  ...
  bool set_selection_parameters( const std::string& param_file ) {
    ...
    try {
      ...
      lower_michel_score = paramset.at("michel_score").at("lower").get<int>();
